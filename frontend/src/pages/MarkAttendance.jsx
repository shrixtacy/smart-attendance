...
import { captureAndSend, generateQR } from "../api/attendance";
...
export default function MarkAttendance() {
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      const data = await generateQR(selectedSubject);
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] p-6 md:p-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-sm text-[var(--text-body)]">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-[var(--text-on-primary)] rounded-lg transition cursor-pointer shadow-sm shadow-blue-200">
              ...
              {activeTab === "All" &&
                Object.entries(attendanceMap).map(([id, s]) => (
                  ...
                  >
                    ...
                    <span
                      className={`px-2 py-0.5 text-[var(--text-on-primary)] text-[10px] rounded-full font-bold ${s.status === "present"
                        ? "bg-[var(--success)]"
                        : "bg-[var(--text-body)]/40"
                        }`}
                    >
      ...
      {showQR && qrToken && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white p-8 rounded-2xl max-w-sm w-full text-center space-y-6 relative shadow-2xl animate-in zoom-in-95 duration-200">
            <button
              onClick={() => setShowQR(false)}
              className="absolute top-4 right-4 p-2 hover:bg-gray-100 rounded-full text-gray-500 transition"
            >
              <X size={20} />
            </button>

            <div>
              <h3 className="text-xl font-bold text-gray-900">Scan to mark attendance</h3>
              <p className="text-gray-500 text-sm mt-1">
                Valid for <span className="text-blue-600 font-bold text-lg tabular-nums">{qrTimer}s</span>
              </p>
            </div>
            ...
            <p className="text-xs text-gray-400">
              Students should scan this using the app.
            </p>
          </div>
        </div>
      )}
